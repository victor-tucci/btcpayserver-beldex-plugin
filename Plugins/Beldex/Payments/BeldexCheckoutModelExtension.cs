...
namespace BTCPayServer.Plugins.Beldex.Payments
{
    public class BeldexCheckoutModelExtension : ICheckoutModelExtension
    {
        ...
        {
            if (context is not { Handler: BeldexLikePaymentMethodHandler handler })
            {
                return;
            }
            ...
            var details = context.InvoiceEntity.GetPayments(true)
                    .Select(p => p.GetDetails<BeldexLikePaymentData>(handler))
                    .Where(p => p is not null)
                    .FirstOrDefault();
            if (details is not null)
            {
                context.Model.ReceivedConfirmations = details.ConfirmationCount;
                context.Model.RequiredConfirmations = (int)BeldexListener.ConfirmationsRequired(details, context.InvoiceEntity.SpeedPolicy);
            }
